...
// The orgId axis above was a `checkKnownGap` for exactly as long as the gap
// existed: v1's `nullifierOf(sec, orgId, period)` let a phantom org buy the
// same credential a second report per epoch. v2 dropped the operand, B.2
// executes the attack, and it is now a plain `checkRejects`. Promoted, not
// deleted — which is what the harness's `checkKnownGap` forces by failing the
// moment its property starts holding.
