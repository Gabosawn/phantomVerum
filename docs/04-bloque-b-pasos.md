...
## Estado — actualizado vie 7/8 ~16:35

| Fase | Estado |
|---|---|
| **A' — contrato** | ✅ A.1–A.6 (pure circuits ejercitados vía selftest / harness de tests) |
| **F0 — toolchain** | ✅ 0.1–0.7 + 0.9 · ⚠️ **0.8** proof server sigue en `:latest` (falta pin `:8.1.0`) |
| **B1 — config/providers** | ✅ |
...
Bloque D (suite + simulate) y los fixes de harness/`noexec`.

> ⚠️ Pendiente admin (Gabriel): topic del repo —
> `gh repo edit Gabosawn/phantomVerum --add-topic midnightntwrk`.
...
- [x] **0.3** Verificar versiones publicadas HOY con `npm view <pkg> version`
...
- [x] **0.4** `app/package.json`: deps pinneadas EXACTAS (sin `^`),
...
- [x] **0.5** `app/tsconfig.json` y `tests/tsconfig.json`:
...
- [x] **0.6** `package.json` raíz: `build`/`test`/`compile` con
      `--workspaces --if-present`; borrar script `lint`.
      ✓ `npm run build` no aborta.
- [x] **0.7** `.env.example` en raíz: `DEPLOY_SEED=` (vacío, con comentario de
...
- [x] **0.9** `npm install` en raíz (con Node 22 activo) y commitear
...
- [x] **A.5** Gate anti-DQ: `contracts/` compila desde clone limpio.
      ...
      Contrato ya en `origin/main` (merge Bloque A).
- [x] **A.6** Smoke del TS generado: cubierto por selftests de `app/`
      (`witnesses/selftest*.ts`, `api/selftest-simulador.ts`) y por el harness
      de `tests/` que importa `contracts/output/contract/index.js` y ejercita
      pure circuits + circuitos. ✓ `npm test` / selftest simulador verdes.

## Fase B1 — Config y providers (`app/src/config/`)

Empieza cuando F0 y A' están verdes. Owner: agente W-app (oleada 2).

- [x] **B1.1** `config/networks.ts`: los dos entornos —
      ...
      config activa.
- [x] **B1.2** `config/deployment.ts` + `config/deployment.json` (placeholder
      `{ "network": null, "contractAddress": null }`): tipo, lector y writer.
      Formato congelado en `03-plan-ejecucion.md` §3.2. ✓ `tsc --noEmit`.
- [x] **B1.3** `config/providers.ts`: construir el objeto de providers de
...
- [x] **B1.4** `setNetworkId` según red activa, una sola vez, en un módulo
...
- [x] **B2.1** `witnesses/secrets.ts`: leer/escribir `secrets/denunciante.json`
...
- [x] **B2.2** `witnesses/evidencia.ts`: hash local del archivo de evidencia
      (`node:crypto` sha-256 → 32 bytes). El archivo NUNCA se sube a ningún
      lado — comentario explícito. ✓ hashea un archivo de prueba,
      determinístico.
- [x] **B2.3** `witnesses/index.ts`: implementar el objeto witnesses que exige
...
- [x] **B3.4** `denunciar({orgId, periodo, evidencia})`: valida credencial vía
...
- [x] **B3.5** `revelarAutoria({denunciaId, fiscalPk})` + error
      `NoSosElAutorError` (proof time). ✓ autor real pasa; secret ajeno falla
      sin tx.
- [x] **B3.6** `verificarAutoria(ExportLlaveAutoria)`: 100 % off-chain —
...
- [x] **B3.7** `leerEstadoLedger()`: query GraphQL al indexer +
...
- [x] **B3.8** Export de llave de autoría: `exportarLlave(denunciaId,
